...
```mermaid
flowchart TD
    user["Пользователь"] --> web["FastAPI и Jinja2"]
    web --> vacancy["Загрузка и разбор вакансии"]
    vacancy --> matching["Расчёт соответствия<br/>и подбор подтверждений"]
    matching --> documents["Резюме и сопроводительное письмо"]
    vacancy --> db[("PostgreSQL")]
    matching --> db
    documents --> db
    llm["DeepSeek-совместимый API<br/>опционально"] -.-> vacancy
    llm -.-> documents
```
