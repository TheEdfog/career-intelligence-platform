...
FastAPI-приложение сравнивает профиль кандидата с вакансией. Оно извлекает требования, показывает подтверждения для рассчитанной оценки соответствия и готовит адаптированное резюме и сопроводительное письмо.

Идея появилась как учебный проект. Текущая версия упакована, покрыта тестами и работает без внешнего AI-сервиса.

Профили, навыки, вакансии, нормализованные требования и созданные документы хранятся в PostgreSQL отдельно. Один профиль можно сравнить с несколькими вакансиями без дублирования исходных данных.

Локальный сценарий разбирает вакансию, рассчитывает соответствие и готовит документы без обращения к внешним сервисам. DeepSeek-совместимый API можно подключить для дополнительной работы с текстом, но основная логика от него не зависит.

## Архитектурные диаграммы

![Общая архитектура платформы](docs/architecture.svg)

Подробные схемы можно открыть по клику:
...
Тесты покрывают маршруты, границы аутентификации, разбор вакансий, расчёт соответствия, рекомендации, профили, интеграцию с hh.ru и резервный сценарий создания документов. Сетевые и LLM-вызовы в тестах заменены заглушками.
